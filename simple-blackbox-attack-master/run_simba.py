...
f = open("googledimages_computationtime.txt", "a")
start = time.time()
torch.cuda.empty_cache()
parser = argparse.ArgumentParser(description='Runs SimBA on a set of images')
parser.add_argument('--data_root', type=str, help='root directory of imagenet data')
...
if os.path.isfile(batchfile):
    ...
    while preds.ne(labels).sum() > 0:
        ...
        for i in list(idx):
            ...
            images[i] = testing[0]
            labels[i] = int(testset.classes[testing[1]])
